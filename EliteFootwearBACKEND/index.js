...
app.use(cors({credentials:true, origin:['https://66a525eac81f7a4de6946cbd--imaginative-platypus-eef631.netlify.app', 'http://localhost:3000']}));
app.use(CookieParser());

// simple helper to propagate async errors to express error handler
...
//update product
app.put("/api/product/:id",asyncHandler(async(req,res)=>{
    try{
        const { id } = req.params;
        const { name, sizes, category, description, price, photos } = req.body;
...
//delete a product
app.delete("/api/product/:id",asyncHandler(async(req,res)=>{
    try{
        const {id}=req.params;
...
app.post('/api/products/search', asyncHandler(async (req, res) => {
...
app.post('/api/exactsearch', asyncHandler(async(req,res)=>{
...
app.post("/api/customer/getfavorites", asyncHandler(async (req, res) => {
    try {
        const arr = req.body;
...
app.post("/api/customer/getcart", asyncHandler(async (req, res) => {
    try {
        const arr = req.body;
...
//update a customer
app.post("/api/particularcustomer", asyncHandler(async (req, res) => {
    try {
        // Parse phone number if present
        if (req.body.phone) {
            const parsedPhone = parseInt(req.body.phone);
            req.body.phone = Number.isNaN(parsedPhone) ? undefined : parsedPhone;
        }
        if (req.body.quantity !== undefined) {
            if (Array.isArray(req.body.quantity)) {
                // Ensure each quantity entry is a valid number
                req.body.quantity = req.body.quantity
                    .map(item => parseInt(item))
                    .filter(item => !Number.isNaN(item));
            } else {
                const parsedQuantity = parseInt(req.body.quantity);
                req.body.quantity = Number.isNaN(parsedQuantity) ? undefined : parsedQuantity;
            }
        }
...
app.post("/api/placeorder", asyncHandler(async(req, res)=>{
    try{
...
app.get("/api/customers/fav", asyncHandler(async (req, res) => {
    try {
        let allfav = [];
...
// central error handler to avoid crashing on rejected promises
...
//connect to database
const mongoUri = process.env.MONGO_URI || "mongodb+srv://sachinrangabaskar344:abcd@backend.4d6ywoo.mongodb.net/?retryWrites=true&w=majority&appName=backend";
